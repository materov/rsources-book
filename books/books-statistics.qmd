...
Здесь приведены книги и ресурсы по статистике.
...
Случайная цитата из [statquotes](https://friendly.github.io/statquotes/){target="_blank"} по темам, связанным со статистикой:

`r as.markdown(statquote("statistics"))`

```{r}
#| echo: false
#| warning: false
statistics <- read_excel("../tables/statistics.xlsx")
...
reactable(
  ...
  columns = list( 
    ...
    title = colDef(
      ...
      sprintf('<a href="%s" target="_blank">%s</a>', statistics$link[index], value)
      }
    ),
    author = colDef(
      sortable = FALSE,
